...
DROP POLICY IF EXISTS "Admins can manage all booking images" ON booking_images;

-- Create new policies for booking_images
...
-- Drop existing storage policies if they exist (SAFE)
DROP POLICY IF EXISTS "Artists can upload booking images" ON storage.objects;
DROP POLICY IF EXISTS "Public can view booking images" ON storage.objects;
DROP POLICY IF EXISTS "Artists can update their booking images" ON storage.objects;
DROP POLICY IF EXISTS "Artists can delete their booking images" ON storage.objects;
...
DO $$
BEGIN
  RAISE NOTICE '✅ Booking system setup completed successfully!';
  RAISE NOTICE '📊 Tables created: bookings, booking_images';
  RAISE NOTICE '🖼️ Storage bucket created: booking-images';
